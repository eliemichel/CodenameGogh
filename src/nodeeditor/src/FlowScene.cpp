...
void
FlowScene::
loadFromMemory(const QByteArray& data, const QPointF position, bool selectNewItems)
{
  ...
  {
    Node & restored = restoreNode(node.toObject());
    NodeGraphicsObject& item = restored.nodeGraphicsObject();
    if (selectNewItems)
    {
    ...
    if (!position.isNull())
    {
        if (offset.isNull())
        {
            offset = position - item.pos() - QPointF(30, 10);
        }
  ...
  {
    SharedConnection restored = restoreConnection(connection.toObject());
    ConnectionGraphicsObject& item = restored->getConnectionGraphicsObject();
    if (selectNewItems)
    {
        item.setSelected(true);
    }
...
void
FlowScene::
loadCopyFromMemory(const QByteArray& data, const QPointF position, bool selectNewItems)
{
	...
	loadFromMemory(QJsonDocument(jsonDocument).toJson(), position, selectNewItems);
}
